class Persona {
  ...
  modificarEdad() {
    if (confirm("¿Desea modificar la edad de la persona?")) {
      ...
      if (
        nuevaEdad === null ||
        nuevaEdad.trim() === "" ||
        !Number.isInteger(Number(nuevaEdad)) ||
        nuevaEdad < 0 ||
        nuevaEdad > 120
      ) {
  ...
  modificarTelefono() {
    if (confirm("¿Desea modificar el número de teléfono de la persona?")) {
      ...
      if (
        nuevoTelefono === null ||
        nuevoTelefono.trim() === "" ||
        !regexTelefono.test(nuevoTelefono)
      ) {
  ...
  modificarProvincia() {
    if (confirm("¿Desea modificar la provincia de la persona?")) {
      ...
      if (
        nuevaProvincia === null ||
        nuevaProvincia.trim() === "" ||
        !provinciasValidas.includes(nuevaProvincia)
      ) {
  ...
  agregarHistorial() {
    ...
    alert(
      "Información agregada correctamente. Número de identificación: " + ID
    );
  }
  ...
  modificarHistorial() {
    ...
    if (nuevoAnio === null || nuevoAnio.trim() === "")
      nuevoAnio = "No especificado";
    if (nuevoTrabajo === null || nuevoTrabajo.trim() === "")
      nuevoTrabajo = "No especificado";
    if (nuevoHobby === null || nuevoHobby.trim() === "")
      nuevoHobby = "No especificado";
