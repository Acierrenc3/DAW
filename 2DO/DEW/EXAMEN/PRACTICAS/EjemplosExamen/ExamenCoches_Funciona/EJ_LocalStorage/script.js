class Vehiculo {
  ...
  static deserializar(serializado) {
    ...
    vehiculo.historial = datos.historial; // Asegúrate de que el historial también se recupere
    return vehiculo;
  }
}
...
function crearNuevo() {
  try {
    ...
    localStorage.setItem("vehiculoActual", vehiculoActual.serializar());
  } catch (error) {
    alert(error.message);
  }
}
...
function cargarVehiculoDesdeLocalStorage() {
  const vehiculoSerializado = localStorage.getItem("vehiculoActual");
  if (vehiculoSerializado) {
    vehiculoActual = Vehiculo.deserializar(vehiculoSerializado);
    document.getElementById(
      "datoscoche"
    ).value = `NIV: ${vehiculoActual.niv}\nMarca: ${vehiculoActual.marca}\nModelo: ${vehiculoActual.modelo}\nColor: ${vehiculoActual.color}\nAño Fabricacion: ${vehiculoActual.af}`;
  }
...
window.addEventListener("load", cargarVehiculoDesdeLocalStorage);
