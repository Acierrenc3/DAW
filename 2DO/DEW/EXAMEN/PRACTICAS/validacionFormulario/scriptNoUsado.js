...
function iniciar() {
  ...
  function confirmarEnvio(event) {
    ...
    if (confirmacion) {
      // Incrementamos el contador de intentos en la cookie
      const intentos = parseInt(getCookie("intentos")) || 0;
      setCookie("intentos", intentos + 1);

      // Se actualiza el contenido del contenedor "intentos"
      const intentosContainer = document.getElementById("intentos");
      intentosContainer.innerHTML = `Intento de Envíos del formulario: ${
        intentos + 1
      }`;
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      if (
        ...
        // No se hace nada si hay errores
      } else {
        confirmarEnvio(event); // Pedir confirmación antes de enviar
      }
    });

  document.getElementById("enviar").addEventListener("click", function (event) {
    validarNombreYApellidos(); // Validar al hacer clic en enviar
